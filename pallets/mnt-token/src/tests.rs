...
fn test_market_list_manipulation() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(MntToken::add_market(Origin::root(), new_market));
		...
		assert_noop!(
			MntToken::add_market(Origin::root(), new_market),
		...
		assert_noop!(
			MntToken::remove_market(Origin::root(), new_market),
