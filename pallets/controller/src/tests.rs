...
fn deposit_interest_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(Controller::deposit_interest(alice(), DOT, 100));
			let expected_event = Event::controller(crate::Event::DepositedInterest(DOT, 100));
			assert!(System::events().iter().any(|record| record.event == expected_event));

			assert_eq!(TestPools::get_pool_total_protocol_interest(DOT), 100);
		});
}
...
fn do_deposit_interest_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(Controller::do_deposit_interest(&ALICE, DOT, 60));
			assert_eq!(TestPools::get_pool_total_protocol_interest(DOT), 60);
			assert_eq!(Currencies::free_balance(DOT, &ALICE), 40);
			assert_eq!(TestPools::get_pool_available_liquidity(DOT), 60);

			// ALICE deposit 5 DOT in pool interest
			assert_ok!(Controller::do_deposit_interest(&ALICE, DOT, 5));
			assert_eq!(TestPools::get_pool_total_protocol_interest(DOT), 65);
			assert_eq!(Currencies::free_balance(DOT, &ALICE), 35);
			assert_eq!(TestPools::get_pool_available_liquidity(DOT), 65);

			// Not enough balance to deposit interest.
			assert_noop!(
				Controller::do_deposit_interest(&ALICE, DOT, 101),
				Error::<Runtime>::NotEnoughBalance
			);

			// There is no pool 'MDOT'.
			assert_noop!(
				Controller::do_deposit_interest(&ALICE, MDOT, 5),
				Error::<Runtime>::PoolNotFound
			);

			// Set BTC pool total interest = max_value()
			TestPools::set_pool_total_protocol_interest(BTC, Balance::max_value());

			// Overflow in calculation: total_protocol_interest = max_value() + 50
			assert_err!(
				Controller::do_deposit_interest(&BOB, BTC, 50),
...
fn redeem_interest_should_work() {
	ExtBuilder::default()
		.user_balance(ALICE, DOT, ONE_HUNDRED)
		.pool_total_protocol_interest(DOT, 1000)
		.build()
		.execute_with(|| {
			// ALICE redeem 100 DOT from pool interest.
			assert_ok!(Controller::redeem_interest(alice(), DOT, 125));
			let expected_event = Event::controller(crate::Event::RedeemedInterest(DOT, 125));
			assert!(System::events().iter().any(|record| record.event == expected_event));

			assert_eq!(TestPools::get_pool_total_protocol_interest(DOT), 875);
			assert_eq!(Currencies::free_balance(DOT, &TestPools::pools_account_id()), 875,);
		});
}
...
fn do_redeem_interest_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(Controller::do_redeem_interest(&ALICE, DOT, 150));
			assert_eq!(TestPools::get_pool_total_protocol_interest(DOT), 850);
			assert_eq!(Currencies::free_balance(DOT, &ALICE), 250);
			assert_eq!(TestPools::get_pool_available_liquidity(DOT), 850);

			// ALICE redeem 300 DOT from pool interest
			assert_ok!(Controller::do_redeem_interest(&ALICE, DOT, 300));
			assert_eq!(TestPools::get_pool_total_protocol_interest(DOT), 550);
			assert_eq!(Currencies::free_balance(DOT, &ALICE), 550);
			assert_eq!(TestPools::get_pool_available_liquidity(DOT), 550);

			// Not enough balance to redeem interest: 550 - 600 < 0
			assert_noop!(
				Controller::do_redeem_interest(&ALICE, DOT, 600),
				Error::<Runtime>::NotEnoughBalance
			);

			// There is no pool 'MDOT'.
			assert_noop!(
				Controller::do_redeem_interest(&ALICE, MDOT, 5),
				Error::<Runtime>::PoolNotFound
			);

			// Set DOT pool total interest = max_value()
			TestPools::set_pool_total_protocol_interest(DOT, Balance::max_value());
			// Overflow in calculation: total_protocol_interest = max_value() + 50
			assert_err!(
				Controller::do_deposit_interest(&ALICE, DOT, 50),
				Error::<Runtime>::BalanceOverflow
			);
		});
}
