...
fn create_pool_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				TestProtocol::create_pool(bob_origin(), DOT, PoolInitData { ..Default::default() }),
				BadOrigin,
			);
			...
			assert_noop!(
				TestProtocol::create_pool(alice_origin(), DOT, create_dummy_pool_init_data()),
...
fn protocol_operations_not_working_for_nonexisting_pool() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				TestProtocol::deposit_underlying(alice_origin(), ETH, dollars(60_u128)),
				liquidity_pools::Error::<Test>::PoolNotFound
			);

			assert_noop!(
				TestProtocol::redeem(alice_origin(), ETH),
				liquidity_pools::Error::<Test>::PoolNotFound
			);

			assert_noop!(
				TestProtocol::redeem_underlying(alice_origin(), ETH, dollars(60_u128)),
				liquidity_pools::Error::<Test>::PoolNotFound
			);

			assert_noop!(
				TestProtocol::redeem_wrapped(alice_origin(), METH, dollars(60_u128)),
				liquidity_pools::Error::<Test>::PoolNotFound
			);

			assert_noop!(
				TestProtocol::borrow(alice_origin(), ETH, dollars(60_u128)),
				liquidity_pools::Error::<Test>::PoolNotFound
			);

			assert_noop!(
				TestProtocol::repay(alice_origin(), ETH, Balance::zero()),
				liquidity_pools::Error::<Test>::PoolNotFound
			);

			assert_noop!(
				TestProtocol::repay_all(alice_origin(), ETH),
				liquidity_pools::Error::<Test>::PoolNotFound
			);

			assert_noop!(
				TestProtocol::repay_on_behalf(bob_origin(), ETH, ALICE, dollars(10_u128)),
				liquidity_pools::Error::<Test>::PoolNotFound
			);

			assert_noop!(
				TestProtocol::enable_is_collateral(alice_origin(), ETH),
				liquidity_pools::Error::<Test>::PoolNotFound
			);

			assert_noop!(
				TestProtocol::disable_is_collateral(alice_origin(), ETH),
				liquidity_pools::Error::<Test>::PoolNotFound
			);

			assert_noop!(
				TestProtocol::transfer_wrapped(alice_origin(), BOB, METH, dollars(10_u128)),
				liquidity_pools::Error::<Test>::PoolNotFound
			);

			assert_noop!(
				TestProtocol::claim_mnt(alice_origin(), vec![DOT, ETH]),
				liquidity_pools::Error::<Test>::PoolNotFound
			);
		});
}
...
fn deposit_underlying_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				TestProtocol::deposit_underlying(alice_origin(), MDOT, 10),
				Error::<Test>::NotValidUnderlyingAssetId
			);

			// Alice has 100 ETH on her account, so she cannot make a deposit 150 ETH.
			assert_noop!(
				TestProtocol::deposit_underlying(alice_origin(), ETH, dollars(150_u128)),
				Error::<Test>::NotEnoughLiquidityAvailable
			);

			// Transaction with zero balance is not allowed.
			assert_noop!(
				TestProtocol::deposit_underlying(alice_origin(), DOT, Balance::zero()),
...
fn redeem_should_not_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				TestProtocol::redeem(bob_origin(), DOT),
				Error::<Test>::NotEnoughWrappedTokens
			);

			// MDOT is wrong CurrencyId for underlying assets.
			assert_noop!(
				TestProtocol::redeem(alice_origin(), MDOT),
				Error::<Test>::NotValidUnderlyingAssetId
			);

			// All operations in the KSM pool are paused.
			assert_noop!(
				TestProtocol::redeem(alice_origin(), KSM),
				Error::<Test>::OperationPaused
			);
...
fn redeem_fails_if_low_balance_in_pool() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				TestProtocol::redeem(alice_origin(), BTC),
				Error::<Test>::NotEnoughLiquidityAvailable
			);
		});
}
...
fn redeem_underlying_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				TestProtocol::redeem_underlying(alice_origin(), DOT, dollars(100_u128)),
				Error::<Test>::NotEnoughWrappedTokens
			);

			// MDOT is wrong CurrencyId for underlying assets.
			assert_noop!(
				TestProtocol::redeem_underlying(alice_origin(), MDOT, dollars(20_u128)),
				Error::<Test>::NotValidUnderlyingAssetId
			);

			// Transaction with zero balance is not allowed.
			assert_noop!(
				TestProtocol::redeem_underlying(alice_origin(), DOT, Balance::zero()),
				Error::<Test>::ZeroBalanceTransaction
			);

			// All operations in the KSM pool are paused.
			assert_noop!(
				TestProtocol::redeem_underlying(alice_origin(), KSM, dollars(10_u128)),
				Error::<Test>::OperationPaused
			);
...
fn redeem_wrapped_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				TestProtocol::redeem_wrapped(alice_origin(), MDOT, dollars(100_u128)),
				Error::<Test>::NotEnoughWrappedTokens
			);

			// MDOT is wrong CurrencyId for underlying assets.
			assert_noop!(
				TestProtocol::redeem_wrapped(alice_origin(), DOT, dollars(20_u128)),
				Error::<Test>::NotValidWrappedTokenId
			);

			// Transaction with zero balance is not allowed.
			assert_noop!(
				TestProtocol::redeem_wrapped(alice_origin(), MDOT, Balance::zero()),
				Error::<Test>::ZeroBalanceTransaction
			);

			// All operations in the KSM pool are paused.
			assert_noop!(
				TestProtocol::redeem_wrapped(alice_origin(), MKSM, dollars(10_u128)),
				Error::<Test>::OperationPaused
			);
...
fn borrow_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				TestProtocol::borrow(alice_origin(), DOT, dollars(100_u128)),
				controller::Error::<Test>::InsufficientLiquidity
			);

			// MDOT is wrong CurrencyId for underlying assets.
			assert_noop!(
				TestProtocol::borrow(alice_origin(), MDOT, dollars(60_u128)),
				Error::<Test>::NotValidUnderlyingAssetId
			);

			// Transaction with zero balance is not allowed.
			assert_noop!(
				TestProtocol::borrow(alice_origin(), DOT, Balance::zero()),
				Error::<Test>::ZeroBalanceTransaction
			);

			// All operations in the KSM pool are paused.
			assert_noop!(
				TestProtocol::borrow(alice_origin(), KSM, dollars(10_u128)),
				Error::<Test>::OperationPaused
			);
...
fn repay_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			// MDOT is wrong CurrencyId for underlying assets.
			assert_noop!(
				TestProtocol::repay(alice_origin(), MDOT, dollars(10_u128)),
				Error::<Test>::NotValidUnderlyingAssetId
			);

			// Alice cannot repay 100 DOT, because she only have 70 DOT.
			assert_noop!(
				TestProtocol::repay(alice_origin(), DOT, dollars(100_u128)),
				Error::<Test>::NotEnoughUnderlyingAsset
			);

			// Alice cannot repay 70 DOT, because she only borrowed 60 DOT.
			assert_noop!(
				TestProtocol::repay(alice_origin(), DOT, dollars(70_u128)),
				liquidity_pools::Error::<Test>::RepayAmountTooBig
			);

			// Transaction with zero balance is not allowed.
			assert_noop!(
				TestProtocol::repay(alice_origin(), DOT, Balance::zero()),
				Error::<Test>::ZeroBalanceTransaction
			);

			// All operations in the KSM pool are paused.
			assert_noop!(
				TestProtocol::repay(alice_origin(), KSM, dollars(10_u128)),
				Error::<Test>::OperationPaused
			);
...
fn repay_all_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			// MDOT is wrong CurrencyId for underlying assets.
			assert_noop!(
				TestProtocol::repay_all(alice_origin(), MDOT),
				Error::<Test>::NotValidUnderlyingAssetId
			);

			// All operations in the KSM pool are paused.
			assert_noop!(
				TestProtocol::repay_all(alice_origin(), KSM),
				Error::<Test>::OperationPaused
			);
...
fn repay_all_fails_if_not_enough_underlying_assets() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(TestProtocol::deposit_underlying(alice_origin(), DOT, dollars(70_u128)));

			// Insufficient DOT in the ALICE account for repay 30 DOT.
			assert_noop!(
				TestProtocol::repay_all(alice_origin(), DOT),
				Error::<Test>::NotEnoughUnderlyingAsset
			);
		});
}
...
fn repay_on_behalf_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			// MDOT is wrong CurrencyId for underlying assets.
			assert_noop!(
				TestProtocol::repay_on_behalf(bob_origin(), MDOT, ALICE, dollars(10_u128)),
				Error::<Test>::NotValidUnderlyingAssetId
			);

			// Bob can't pay off the 120 DOT debt for Alice, because he has 100 DOT in his account.
			assert_noop!(
				TestProtocol::repay_on_behalf(bob_origin(), DOT, ALICE, dollars(120_u128)),
				Error::<Test>::NotEnoughUnderlyingAsset
			);

			// Bob cannot repay 100 DOT, because Alice only borrowed 60 DOT.
			assert_noop!(
				TestProtocol::repay_on_behalf(bob_origin(), DOT, ALICE, dollars(100_u128)),
				liquidity_pools::Error::<Test>::RepayAmountTooBig
			);

			// Transaction with zero balance is not allowed.
			assert_noop!(
				TestProtocol::repay_on_behalf(bob_origin(), DOT, ALICE, Balance::zero()),
				Error::<Test>::ZeroBalanceTransaction
			);

			// All operations in the KSM pool are paused.
			assert_noop!(
				TestProtocol::repay_on_behalf(bob_origin(), KSM, ALICE, dollars(10_u128)),
				Error::<Test>::OperationPaused
			);
...
fn enable_is_collateral_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				TestProtocol::enable_is_collateral(alice_origin(), MDOT),
				Error::<Test>::NotValidUnderlyingAssetId
			);
		});
}
...
fn disable_is_collateral_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				TestProtocol::disable_is_collateral(alice_origin(), MDOT),
				Error::<Test>::NotValidUnderlyingAssetId
			);
		});
}
...
fn transfer_wrapped_should_not_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				TestProtocol::transfer_wrapped(alice_origin(), BOB, MDOT, dollars(101_u128)),
				Error::<Test>::NotEnoughWrappedTokens
			);

			// Bob is unable to transfer tokens with zero balance
			assert_noop!(
				TestProtocol::transfer_wrapped(bob_origin(), ALICE, MDOT, 1_u128),
				Error::<Test>::NotEnoughWrappedTokens
			);

			// Bob is unable to send zero tokens
			assert_noop!(
				TestProtocol::transfer_wrapped(bob_origin(), ALICE, MBTC, Balance::zero()),
				Error::<Test>::ZeroBalanceTransaction
			);
...
fn claim_mnt_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(TestProtocol::deposit_underlying(bob_origin(), DOT, dollars(100_u128)));
			assert_ok!(TestProtocol::borrow(bob_origin(), DOT, dollars(50_u128)));
			...
			assert_ok!(TestProtocol::claim_mnt(alice_origin(), vec![DOT]));
			...
			assert_ok!(TestProtocol::borrow(alice_origin(), DOT, dollars(10_u128)));
