...
type LiquidationAmountsResult = Result<(Vec<(OriginalAsset, Balance)>, Vec<(OriginalAsset, Balance)>), DispatchError>;
	...
	T: Config + Debug,
{
	...
	borrows_to_repay_underlying: Vec<(OriginalAsset, Balance)>,
	...
	supplies_to_seize_underlying: Vec<(OriginalAsset, Balance)>,
	...
	supplies_to_pay_underlying: Option<Vec<(OriginalAsset, Balance)>>,
...
impl<T: Config + Debug> UserLoanState<T> {
	...
	pub fn get_user_borrows_to_repay_underlying(&self) -> Vec<(OriginalAsset, Balance)> {
		self.borrows_to_repay_underlying.clone()
	}

	/// Getter for `self.supplies_to_seize_underlying`.
	pub fn get_user_supplies_to_seize_underlying(&self) -> Vec<(OriginalAsset, Balance)> {
		self.supplies_to_seize_underlying.clone()
	}

	/// Getter for `self.supplies_to_pay_underlying`.
	pub fn get_user_supplies_to_pay_underlying(&self) -> Option<Vec<(OriginalAsset, Balance)>> {
