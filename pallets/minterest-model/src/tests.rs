	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				...
				Error::<TestRuntime>::BaseRatePerBlockCannotBeZero
			);
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				...
				Error::<TestRuntime>::MultiplierPerBlockCannotBeZero
			);
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				...
				Error::<TestRuntime>::KinkCannotBeMoreThanOne
			);
		});
}
...
fn calculate_pool_borrow_interest_rate_fails_if_overflow_kink_mul_multiplier() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			<MinterestModelDataStorage<TestRuntime>>::insert(KSM, minterest_model_data.clone());
			...
			assert_noop!(
				...
				Error::<TestRuntime>::BorrowRateCalculationError
			);
		});
}
