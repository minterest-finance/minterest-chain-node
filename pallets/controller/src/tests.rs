...
fn convert_to_wrapped_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(Controller::convert_to_wrapped(DOT, 10), Ok(25));

			// Overflow in calculation: wrapped_amount = max_value() / exchange_rate,
			// when exchange_rate < 1
			assert_err!(
				Controller::convert_to_wrapped(DOT, Balance::max_value()),
				Error::<Runtime>::ConversionError
			);
		});
}
...
fn convert_from_wrapped_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(Controller::convert_from_wrapped(MDOT, 10), Ok(4));

			// Overflow in calculation: underlying_amount = max_value() * exchange_rate
			assert_err!(
				Controller::convert_from_wrapped(MBTC, Balance::max_value()),
				Error::<Runtime>::ConversionError
			);
		});
}
...
fn calculate_exchange_rate_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			...
			Error::<Runtime>::ExchangeRateCalculationError
		);
...
fn get_exchange_rate_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(Controller::get_exchange_rate(ETH), Error::<Runtime>::PoolNotFound);
			// exchange_rate = (100 - 0 + 300) / 125 = 3.2
			assert_eq!(
				Controller::get_exchange_rate(DOT),
...
fn get_is_collateral_pools_should_work() {
	ExtBuilder::default()
		...
		.pool_user_data(KSM, ALICE, Balance::zero(), Rate::default(), true, 0)
		.pool_user_data(DOT, ALICE, Balance::zero(), Rate::default(), true, 0)
		.pool_user_data(ETH, ALICE, Balance::zero(), Rate::default(), true, 0)
		.pool_user_data(BTC, ALICE, Balance::zero(), Rate::default(), false, 0)
		...
		.build()
		.execute_with(|| {
			assert_eq!(Controller::get_is_collateral_pools(&ALICE), Ok(vec![DOT, ETH]));
			assert_eq!(Controller::get_is_collateral_pools(&BOB), Ok(vec![]));
		});
}
